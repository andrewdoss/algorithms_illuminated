...
def built_in_search(x, target):
    ...
    Note: This should be slower than binary because it can't assume sorted input.
    '''
    try:
        idx = x.index(target)
    except ValueError:
...
def get_worst_case_setup(algorithm_name, n):
    s = f"from __main__ import {algorithm_name};"
    s += f"x = list(range({n}));"
    s += f"target = {n + 1}"
    return s

def get_random_setup(algorithm_name, n):
    s = f"from __main__ import {algorithm_name}; import numpy as np;"
    s += f"x = list(range({n}));"
    s += f"target = np.random.randint(0,{n},1)[0]"
    return s 
...
if __name__ == '__main__':
    ...
    results = pd.DataFrame(data={'n':[1000000, 10000000, 100000000]})
    for algorithm_name, algorithm in algorithms.items():
        print(f'\nRunning worst-case timing tests for {algorithm_name}...')
        temp_results = []
        for n in results['n'].values:
            temp_results.append(np.round(timeit.timeit(f'{algorithm_name}(x, target)', setup=get_worst_case_setup(algorithm_name, n), number=1), 4))
    ...
    for algorithm_name, algorithm in algorithms.items():
        ...
        for n in results['n'].values:
            ...
            for trial in range(num_trials):
                trial_results.append(np.round(timeit.timeit(f'{algorithm_name}(x, target)', setup=get_random_setup(algorithm_name, n), number=1), 4))
            temp_results.append(np.mean(trial_results))
        results[algorithm_name] = temp_results
    print('Timing tests complete.')
    print(results.head(results.shape[0]))