...
from chapter_1.sorting import selection_sort
...
def partition(x, left, right, comparison_counter=None):
    ...
    Args:
        ...
        left (int) : The index for the start of the sub-list within x.
        right (int) : The index for the end of the sub-list within x.
        
    Returns:
        int : The final pivot position.
    '''
...
def quicksort(x, left=None, right=None, select_pivot=uniform_random_pivot, comparison_counter=None):
    ...
    Args:
        x (list): A list to be sorted in place.
        left (int): The beginning of the current sub-list for sorting.
        right (int): The end of the current sub-list for sorting.
        select_pivot (func): A function for choosing pivots with signature (x, left, right).
        comparison_count (list): An optional list with single int for holding a comparison counter.
    ...
    # Recursive calls
    quicksort(x, left, j - 1, select_pivot, comparison_counter)
    quicksort(x, j + 1, right, select_pivot, comparison_counter)
...
def func_wrapper(method, x, comparison_counter=None):
    if method == 'selection_sort':
        ...
        return quicksort(x, select_pivot=first_pivot, comparison_counter=comparison_counter)
    elif method == 'qs_last':
        return quicksort(x, select_pivot=last_pivot, comparison_counter=comparison_counter)
    elif method == 'qs_random':
        return quicksort(x, select_pivot=uniform_random_pivot, comparison_counter=comparison_counter)
    elif method == 'qs_med_of_3':
        return quicksort(x, select_pivot=median_of_three_pivot, comparison_counter=comparison_counter)
    else:
...
if __name__ == '__main__':
    ...
    for input_type in ['presorted', 'reversed', 'random', 'shuffled']:
        for method in ['qs_first', 'qs_last', 'qs_random', 'qs_med_of_3']:
            x = get_input(input_type, 500) # The first/last methods w/ pre-sorted limits input size due to recursion depth
            x_c = x.copy()
